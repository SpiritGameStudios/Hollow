...
public abstract class CopperBulbBlockMixin extends Block {
    @Shadow public abstract void checkAndFlip(BlockState state, ServerLevel level, BlockPos pos);
    ...
    private void scheduleDelayedTick(CopperBulbBlock instance, BlockState state, ServerLevel level, BlockPos pos, Operation<Void> original) {
        if (level.getGameRules().get(HollowGameRules.COPPER_BULB_DELAY)) {
            level.scheduleTick(pos, instance, 1);
        } else {
