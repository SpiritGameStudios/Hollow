...
public record ServerboundPropelFurnaceBoatPayload(int entity, boolean propelled) implements CustomPacketPayload {
	public static final StreamCodec<FriendlyByteBuf, ServerboundPropelFurnaceBoatPayload> STREAM_CODEC = StreamCodec.composite(
		ByteBufCodecs.VAR_INT, ServerboundPropelFurnaceBoatPayload::entity,
	...
	public static class Receiver implements ServerPlayNetworking.PlayPayloadHandler<ServerboundPropelFurnaceBoatPayload> {
		...
		public void receive(ServerboundPropelFurnaceBoatPayload payload, ServerPlayNetworking.Context context) {
			if (context.player().level().getEntity(payload.entity) instanceof AbstractFurnaceBoat furnaceBoat) {
